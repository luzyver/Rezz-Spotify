<script lang="ts">
  ...
  import { filterAndSortHistory } from '$lib/stores/filters';
  ...
  let activeTab = $state<'home' | 'recent' | 'history' | 'insights' | 'achievements' | 'calendar'>('home');
  ...
  let filteredCombinedStore = $derived(filterAndSortHistory(combinedHistory));
  let filteredCombinedHistory = $state<HistoryItem[]>([]);

  $effect(() => {
    const unsubscribe = filteredCombinedStore.subscribe((value) => {
      filteredCombinedHistory = value;
      currentPage = 1;
    });
    return unsubscribe;
  });
          ...
          {#if allHistory.length > 0}
            <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
              {#each allHistory as item, index (item.uri + ':' + item.timestamp)}
                <HistoryCard {item} {index} currentPage={1} itemsPerPage={allHistory.length} viewMode="grid" />
              {/each}
            </div>
          {:else}
            <div class="py-16 text-center">
